...
- `birtir::md_text()` prints Markdown-ready inline text with glue
  formatting and LaTeX math support
- `birtir::fmt_num()` returns formatted numbers for inline reporting
- `birtir::fmt_p()` returns APA-style p-value strings such as `= .023`
  or `< .001`
- `birtir::formula_to_latex()` converts R formulas into compact LaTeX
  equations
- `birtir::lavaan_to_latex()` converts CFA and SEM lavaan model syntax
  into LaTeX equations
- `birtir::convert_md()` converts an existing `.md` file with Pandoc

## LaTeX math and formulas

`md_text()` supports standard LaTeX math formulas (`$...$` for inline
math and `$$...$$` for display math). You can also dynamically convert R
model formulas and `lavaan` CFA/SEM syntax into LaTeX equations using
`formula_to_latex()` and `lavaan_to_latex()`:
